...
function MatchCard({ match, navigate }: { match: MatchListItem; navigate: (path: string) => void }) {
...
function Section({ title, items, navigate }: { title: string; items: MatchListItem[]; navigate: (path: string) => void }) {
  ...
  return (
    <div style={{ marginBottom: 32 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 12 }}>
        {items.map(m => <MatchCard key={m.id} match={m} navigate={navigate} />)}
      </div>
    </div>
  );
}
...
export default function Discover({ navigate }: Props) {
  ...
  return (
    <div style={{ maxWidth: 1280, margin: '0 auto', padding: '32px 24px' }}>
      ...
      <Section title="Public Matches" items={publicMatches} navigate={navigate} />
      <Section title="Invited to Watch" items={invitedMatches} navigate={navigate} />
    </div>
  );
}
